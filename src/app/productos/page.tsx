...
import { Plus, Search, Filter, Edit, Trash2, MoreHorizontal, FileUp, Loader2, Package, AlertCircle } from "lucide-react"
  ...
  if (isUserLoading || isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#f4f4f5] gap-4">
        ...
        <p className="font-black uppercase italic tracking-tighter">Sincronizando Catálogo...</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 lg:p-10 space-y-8 bg-[#f8f9fa] min-h-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground font-medium mt-1">Gestión de inventario y precios maestros.</p>
          </div>
        </div>
        <div className="flex gap-3">
          ...
          >
            <FileUp className="w-5 h-5 mr-2" /> Importar CSV
          </Button>
          <Button 
            size="lg" 
            onClick={() => {
          ...
          >
            <Plus className="w-6 h-6 mr-2" /> Nuevo Producto
          </Button>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <Button variant="outline" className="h-14 px-6 border-2 border-black font-bold uppercase tracking-tighter rounded-xl flex items-center gap-2">
      ...
      <div className="bg-white rounded-2xl shadow-xl border-none overflow-hidden">
        <Table>
          <TableHeader className="bg-black">
            <TableRow className="hover:bg-black border-none">
              <TableHead className="text-white font-black uppercase text-[10px] tracking-widest h-14 w-[120px]">Código</TableHead>
              <TableHead className="text-white font-black uppercase text-[10px] tracking-widest h-14">Producto</TableHead>
              <TableHead className="text-white font-black uppercase text-[10px] tracking-widest h-14">Marca</TableHead>
              <TableHead className="text-white font-black uppercase text-[10px] tracking-widest h-14">Precio Venta</TableHead>
              <TableHead className="text-white font-black uppercase text-[10px] tracking-widest h-14">Stock</TableHead>
                ...
                <TableCell className="text-right">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon" className="h-10 w-10 hover:bg-primary hover:text-black rounded-lg">
                        <MoreHorizontal className="w-5 h-5" />
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-48 rounded-xl border-2 border-black">
                      <DropdownMenuLabel className="font-black uppercase text-[10px] tracking-widest text-muted-foreground">Opciones</DropdownMenuLabel>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem onClick={() => handleEdit(product)} className="flex items-center gap-2 font-bold cursor-pointer">
        ...
        {filteredProducts.length === 0 && !isLoading && (
          <div className="p-20 text-center flex flex-col items-center gap-4 text-muted-foreground bg-white">
            ...
            <p className="font-black uppercase italic tracking-tighter text-xl">Sin resultados en el catálogo</p>
            <p className="text-xs font-bold uppercase">Añade productos manualmente o mediante importación masiva.</p>
          </div>
        )}
      </div>
