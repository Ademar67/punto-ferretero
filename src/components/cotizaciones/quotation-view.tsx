...
import { Hammer, FileText, User, Phone, Calendar } from "lucide-react"
...
export function QuotationView({ quotation }: QuotationViewProps) {
  ...
  const validUntilDate = quotation.validUntil?.seconds ? new Date(quotation.validUntil.seconds * 1000) : null
    ...
    <div className="w-full bg-white text-black font-body p-8 border-2 border-muted rounded-2xl shadow-sm">
      ...
      <div className="flex justify-between items-start mb-10 pb-6 border-b-2 border-black">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mt-1">Cotizaciones y Presupuestos Profesionales</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
        ...
        <div className="text-right space-y-1">
          <div className="flex justify-end items-center gap-2 text-xs font-bold">
            ...
            <span>{format(quoteDate, "dd/MM/yyyy HH:mm", { locale: es })}</span>
          </div>
          <div className="flex justify-end items-center gap-2 text-xs font-bold text-red-600">
            <span className="text-muted-foreground uppercase">Válido hasta:</span>
            <span className="flex items-center gap-1">
              <Calendar className="w-3 h-3" />
              {validUntilDate ? format(validUntilDate, "dd/MM/yyyy", { locale: es }) : "7 días naturales"}
            </span>
          </div>
          <div className="flex justify-end items-center gap-2 text-xs font-bold">
            <span className="text-muted-foreground uppercase">Atendido por:</span>
          ...
          {quotation.items.map((item, idx) => (
            <div key={idx} className="grid grid-cols-12 p-4 border-b border-muted hover:bg-muted/10 transition-colors">
              <div className="col-span-6">
                ...
                <p className="text-[8px] font-bold text-muted-foreground uppercase">REF: {item.productId.substring(0,8)}</p>
              </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-8">
        <div className="max-w-md space-y-2">
          <span className="text-[10px] font-black uppercase text-muted-foreground italic">Términos y Condiciones:</span>
          <p className="text-[9px] text-muted-foreground leading-relaxed font-medium">
            * Los precios mostrados incluyen IVA del 16%. * Esta cotización tiene una validez de 7 días naturales a partir de su emisión. * La disponibilidad de stock puede variar al momento de concretar la compra. * El pago total debe realizarse para liberar la mercancía.
          </p>
        </div>
      ...
      <div className="mt-16 text-center border-t-2 border-dashed border-muted pt-8">
        <p className="text-[11px] font-black italic uppercase text-teal-600 mb-2">¡Gracias por preferir Punto Ferretero!</p>
        <p className="text-[8px] font-bold text-muted-foreground uppercase tracking-[0.3em]">Sistema de Gestión v1.0 • Documento No Fiscal</p>
      </div>
    </div>
  )
}
